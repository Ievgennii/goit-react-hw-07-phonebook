...
import { addContacts, delContacts, getContacts } from 'redux/operations'; // Импортируем функции addContacts, delContacts и getContacts из файла 'redux/operations'

export const getContactsThunk = createAsyncThunk(
  // Создаем асинхронное Thunk-действие с помощью функции createAsyncThunk
  'contacts/allContacts', // Указываем строку действия (action) для получения всех контактов
  async (_, { reject }) => {
    try {
      const data = getContacts(); // Вызываем функцию getContacts для получения контактов
...
export const delContactsThunk = createAsyncThunk(
  ...
  'contacts/delContact', // Указываем строку действия (action) для удаления контакта
  async (id, { reject }) => {
    try {
